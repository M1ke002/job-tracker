...
@pytest.mark.parametrize("stage_name, has_rejected_at_stage", [
    ('Offer', True),
    ('Rejected', False),
    ('Rejected', True),
])
def test_update_job_stage_successful(mock_saved_job, mock_application_stage, mock_db, stage_name, has_rejected_at_stage):
    ...
    mock_application_stage.query.filter_by.return_value.first.side_effect = [stage, stage_apply]
...
@pytest.mark.parametrize("is_job_found, is_stage_found", [
    (False, True),
    (True, False),
])
